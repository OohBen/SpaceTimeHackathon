---
name: "source-command-hackathon-setup"
...
# source-command-hackathon-setup

Use this skill when the user asks to run the migrated source command `hackathon-setup`.
...
If `.Codex/` already exists, bootstrap is complete — continue to Step 3.

If `.Codex/` is missing (e.g. a teammate cloning the repo, or a manual install):
...
| `HARNESS.md` | Setup guide for non-Codex harnesses (Cursor, Aider, etc.) |
