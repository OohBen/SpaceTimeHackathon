...
function bodyPos(b) {
  ...
  if (b.moonOf) {
    const parent = BODIES.find(p => p.id === b.moonOf);
    const pp = bodyPos(parent);
    const a = (b.moonAngle || 0) * Math.PI / 180;
    return { x: pp.x + Math.cos(a) * b.moonDist, y: pp.y + Math.sin(a) * b.moonDist };
  }
  const a = (ANGLES[b.id] || 0) * Math.PI / 180;
  return { x: CX + Math.cos(a) * b.orbit, y: CY + Math.sin(a) * b.orbit };
}

const SolarMap = ({ selectedBody, onSelectBody, glow, alerts }) => {
  const [hover, setHover] = useStateM(null);
  const [tipPos, setTipPos] = useStateM({ x: 0, y: 0 });
  ...
  return (
    <div className="map-wrap" ref={wrapRef}>
      <svg className="map-svg" viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} preserveAspectRatio="xMidYMid meet">
        ...
        {/* fleet markers */}
        <g opacity="0.95">
          <FleetMark x={bodyPos(BODIES.find(b=>b.id==='mars')).x - 22} y={bodyPos(BODIES.find(b=>b.id==='mars')).y - 18} color="var(--imperium)"/>
          <FleetMark x={bodyPos(BODIES.find(b=>b.id==='ceres')).x + 16} y={bodyPos(BODIES.find(b=>b.id==='ceres')).y - 14} color="var(--accord)"/>
          <FleetMark x={bodyPos(BODIES.find(b=>b.id==='earth')).x - 20} y={bodyPos(BODIES.find(b=>b.id==='earth')).y + 16} color="var(--imperium)"/>
        </g>
      </svg>
