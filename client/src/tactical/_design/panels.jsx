/* ============================================================
   SOLAR DOMINION — Secondary panels
   ============================================================ */
...
const Bar = ({ v, color, max=100 }) => (
  <span className="meter"><i style={{ width: (v/max*100)+'%', background: color||'var(--imperium)' }}></i></span>
);

/* ---------- PERSONNEL ---------- */
const REL_COLOR = { trust:'var(--cyan)', alliance:'var(--imperium-2)', mentor:'var(--ok)', rivalry:'var(--alert)', tension:'var(--warn)' };
const PersonnelPanel = ({ roster, relationships }) => {
      ...
      {Object.values(R).map(p => (
        <div key={p.id} className="person-row">
          ...
          <div className="mini-traits" style={{ gap:9 }}>
            {[['COMP',p.competence],['CREA',p.creativity],['RELY',p.reliability],['AMB',p.ambition],['POL',p.political],['COM',p.comms],['LOY',p.loyalty],['AUT',p.autonomy]].map(([k,v])=>(
      ...
      <div style={{ maxWidth:860, marginTop:18 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
          {rels.map(rel => {
            ...
            const col = REL_COLOR[rel.type] || 'var(--muted)';
            return (
...
const SHIP_STATUS = { preparing:'var(--warn)', in_transit:'var(--cyan)', arrived:'var(--ok)', lost:'var(--alert)' };
const LLM_STATUS = { complete:'var(--ok)', processing:'var(--warn)', queued:'var(--muted)', failed:'var(--alert)' };
const bodyName = (id) => (BODIES.find(b=>b.id===id)||{name:id}).name;
const ResourcesPanel = ({ faction, cities, prodOwners, fleets, ships, projects, llm }) => (
  <div className="panel-view">
    ...
    <div className="pv-grid" style={{ gridTemplateColumns:'repeat(2,1fr)', maxWidth:820 }}>
      ...
      <div className="card" style={{ gridColumn:'1 / -1' }}>
        ...
        {(cities || CITIES.filter(c=>c.faction==='imperium'||c.faction==='contested')).map(c => (
        ...
        {(ships||[]).length ? (ships||[]).map(s => (
          <div key={s.id} style={{ padding:'8px 0', borderBottom:'1px solid var(--hairline)' }}>
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              ...
              <span style={{ marginLeft:'auto', fontFamily:'var(--font-mono)', fontSize:10, letterSpacing:'0.08em', textTransform:'uppercase', color: SHIP_STATUS[s.status] }}>{s.status.replace('_',' ')}</span>
            </div>
      ...
      <div className="card">
        ...
        {(llm||[]).map(r => (
          <div key={r.id} className="kv">
            <span className="k" style={{ display:'flex', alignItems:'center', gap:8 }}><span style={{ width:7, height:7, borderRadius:'50%', background: LLM_STATUS[r.status] }}></span>{r.type.replace('_',' ')}</span>
            <span className="v" style={{ color: LLM_STATUS[r.status], fontSize:11, textTransform:'uppercase', letterSpacing:'0.06em' }}>{r.status}</span>
          </div>
        ))}
      </div>
    </div>
...
/* ---------- DOCTRINE ---------- */
const DoctrinePanel = ({ faction, enemy, selfName, enemyName, selfColor, selfColor2, enemyColor, enemyColor2 }) => {
  const opp = enemy || FACTIONS.accord;
  ...
  return (
    <div className="panel-view">
      ...
      <div className="card" style={{ maxWidth:680 }}>
        ...
        {Object.keys(faction.doctrine).map(k => (
...
/* ---------- DIPLOMACY ---------- */
const DiplomacyPanel = ({ data }) => {
  const d = data || IMP_DIPLO;
  return (
    <div className="panel-view">
      <div style={{ fontFamily:'var(--font-prose)', fontSize:13, color:'var(--muted)', maxWidth:680, marginBottom:16, lineHeight:1.5 }}>
...
const CityDetail = ({ bodyId, onClose }) => {
  const body = BODIES.find(b => b.id === bodyId);
  if (!body) return null;
  const cities = CITIES.filter(c => c.bodyId === bodyId);
  const ctrlColor = { imperium:'var(--imperium)', accord:'var(--accord)', contested:'var(--warn)', neutral:'var(--neutral)', star:'var(--sun)' }[body.control];
  return (
    <div className="reader-scrim" onClick={onClose}>
      <div className="reader" style={{ width:'min(560px,70%)' }} onClick={e=>e.stopPropagation()}>
        ...
        <div className="reader-body">
          ...
          {cities.length>0 ? (<>
            <div className="section-label">Settlements · {cities.length}</div>
            {cities.map(c => {
              const cc = { imperium:'var(--imperium)', accord:'var(--accord)', contested:'var(--warn)' }[c.faction];
              return (
                <div key={c.id} className="card" style={{ marginBottom:10 }}>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12 }}>
                    {[['MORALE',c.morale,c.morale<50?'var(--alert)':'var(--ok)'],['INDUSTRY',c.ind,'var(--accord)'],['RESEARCH',c.res,'var(--cyan)'],['GARRISON',c.garrison,'var(--imperium)']].map(([k,v,col])=>(
...
const EV_COLOR = { a:'var(--imperium)', b:'var(--accord)', n:'var(--muted)' };
const TurnSummaryView = ({ summary, roster }) => {
  const s = summary; const c = s.control;
  const off = (roster || PERSONNEL)[s.officer.from];
  return (
    <>
...
const BriefingReader = ({ briefing, onClose, roster }) => {
  if (!briefing) return null;
  const off = (roster || PERSONNEL)[briefing.from];
  if (!off) return null;
...
Object.assign(window, { PersonnelPanel, ResourcesPanel, DoctrinePanel, IntelPanel, DiplomacyPanel, CityDetail, BriefingReader });
