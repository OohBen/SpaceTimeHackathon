...
import { t as require_base64_js } from "./base64-js-C39t9xMi.js";
//#region ../node_modules/spacetimedb/dist/react/index.mjs
...
var SpacetimeDBContext = (0, import_react.createContext)(void 0);
function useSpacetimeDB() {
	const context = (0, import_react.useContext)(SpacetimeDBContext);
	if (!context) throw new Error("useSpacetimeDB must be used within a SpacetimeDBProvider component. Did you forget to add a `SpacetimeDBProvider` to your component tree?");
	return context;
}
...
var ConnectionManager = new class _ConnectionManagerImpl {
	...
	#notify(managed) {
		for (const listener of managed.listeners) listener();
	}
	/**
	...
	*/
	retain(key, builder) {
		...
		managed.refCount += 1;
		if (managed.connection) return managed.connection;
	...
	release(key) {
		...
		if (!managed) return;
		managed.refCount -= 1;
		if (managed.refCount > 0 || managed.pendingRelease) return;
		managed.pendingRelease = setTimeout(() => {
			managed.pendingRelease = null;
			if (managed.refCount > 0) return;
			if (managed.connection) {
				if (managed.onConnect) managed.connection.removeOnConnect(managed.onConnect);
				if (managed.onDisconnect) managed.connection.removeOnDisconnect(managed.onDisconnect);
				if (managed.onConnectError) managed.connection.removeOnConnectError(managed.onConnectError);
	...
	subscribe(key, listener) {
		...
		return () => {
			...
			if (managed.refCount <= 0 && managed.listeners.size === 0 && !managed.connection) this.#connections.delete(key);
		};
	}
	getSnapshot(key) {
...
function SpacetimeDBProvider({ connectionBuilder, children }) {
	const uri = connectionBuilder.getUri();
	const moduleName = connectionBuilder.getModuleName();
	const key = import_react.useMemo(() => ConnectionManager.getKey(uri, moduleName), [uri, moduleName]);
	const fallbackStateRef = import_react.useRef({
		isActive: false,
		identity: void 0,
		token: void 0,
		connectionId: ConnectionId.random(),
	...
	const subscribe = import_react.useCallback((onStoreChange) => ConnectionManager.subscribe(key, onStoreChange), [key]);
	const getSnapshot = import_react.useCallback(() => ConnectionManager.getSnapshot(key) ?? fallbackStateRef.current, [key]);
	const getServerSnapshot = import_react.useCallback(() => fallbackStateRef.current, []);
	const state = import_react.useSyncExternalStore(subscribe, getSnapshot, getServerSnapshot);
	const getConnection = import_react.useCallback(() => ConnectionManager.getConnection(key), [key]);
	...
	return import_react.createElement(SpacetimeDBContext.Provider, { value: contextValue }, children);
}
...
function useTable(query, callbacks) {
	...
	try {
		...
		throw new Error("Could not find SpacetimeDB client! Did you forget to add a `SpacetimeDBProvider`? `useTable` must be used in the React component tree under a `SpacetimeDBProvider` component.");
	}
	...
	const computeSnapshot = (0, import_react.useCallback)(() => {
		...
		return [whereExpr ? Array.from(table.iter()).filter((row) => evaluateBooleanExpr(whereExpr, row)) : Array.from(table.iter()), subscribeApplied];
	}, [
		connectionState,
		accessorName,
		querySql,
	...
	const subscribe = (0, import_react.useCallback)((onStoreChange) => {
		...
		const onUpdate = (ctx, oldRow, newRow) => {
			switch (classifyMembership(whereExpr, oldRow, newRow)) {
				case "leave":
	...
	const getSnapshot = (0, import_react.useCallback)(() => {
		if (!lastSnapshotRef.current) lastSnapshotRef.current = computeSnapshot();
...
function useReducer(reducerDef) {
	...
	(0, import_react.useEffect)(() => {
		...
		if (queueRef.current.length) {
			...
			for (const item of pending) fn(...item.params).then(item.resolve, item.reject);
		}
	}, [
		getConnection,
		reducerName,
	...
	return (0, import_react.useCallback)((...params) => {
		const conn = getConnection();
		if (!conn) return new Promise((resolve, reject) => {
